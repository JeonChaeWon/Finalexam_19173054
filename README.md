...
해당 프로젝트는 로그인 화면, 메뉴 화면, 일별 박스오피스 화면 이렇게 세 가지로 구성되어 있습니다. <br>
맨 처음 로그인 화면에서 아이디와 비밀번호를 입력하여 로그인 버튼을 클릭하면 메뉴 화면으로 이동하게 됩니다. <br>
메뉴 화면은 일별 박스오피스 버튼과 로그인 화면 버튼으로 구성되어 있습니다. <br>
일별 박스오피스 버튼을 클릭하게 되면 일별 박스오피스 화면으로 이동하고, 로그인 화면 버튼을 클릭하면 로그인 화면으로 이동합니다. <br>
일별 박스오피스 화면은 openAPI 요청 URL을 이용하여 데이터를 가져오고 그 데이터로 일별 박스오피스 목록을 조회합니다.
